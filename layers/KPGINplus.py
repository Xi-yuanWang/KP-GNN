...
class KPGINPlusConv(MessagePassing):
    ...
    def __init__(self,
        ...
        self.mlp = nn.Sequential(nn.Linear(input_size, output_size),
                                 nn.BatchNorm1d(output_size), nn.ReLU(),
                                 nn.Linear(output_size, output_size),
                                 nn.BatchNorm1d(output_size), nn.ReLU())
        self.initial_eps = eps
        if train_eps:
            self.eps = torch.nn.Parameter(
                torch.Tensor([[eps for _ in range(self.K)]]))
        else:
        ...
        self.hop1_edge_emb = torch.nn.Embedding(num_hop1_edge,
                                                input_size,
                                                padding_idx=0)

        #If K larger than 1, define additional embedding and combine function
        if self.K > 1:
            self.hopk_edge_emb = torch.nn.Embedding(num_hopk_edge,
                                                    input_size,
                                                    padding_idx=0)
        ...
        self.efeature_mlp = nn.Sequential(
            nn.Linear(input_size * self.K, input_size * self.K),
    ...
    def forward(self, x, edge_index, edge_attr, peripheral_attr=None):
        ...
        if self.K > 1:
            ...
            e_emb = e1_emb
        E, K, H = e_emb.shape[0], e_emb.shape[1], e_emb.shape[2]
        e_emb = self.efeature_mlp(e_emb.flatten(-2, -1)).reshape(E, K, H)

        x_n = self.propagate(edge_index, x=x, edge_attr=e_emb,
                             mask=edge_attr)  # N * K * H
        ...
        if peripheral_attr is not None:
            ...
            total = torch.sum((peripheral_attr > 0).int(),
                              dim=-1).unsqueeze(-1)  # N * K * c * 1
            total[total == 0] = 1
            se_emb = se_emb / total
...
class KPGINPlusConvOGBMol(MessagePassing):
    ...
    def forward(self,
        ...
        if peripheral_attr is not None:
            se_emb = self.hop1_edge_emb(peripheral_attr)  # N * K * c * E' * dk
            mask = (torch.sum(peripheral_attr, dim=-1) > 0).int().unsqueeze(
                -1)  # N * K * c * E' * 1
